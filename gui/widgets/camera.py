...
class CameraCard(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        ctk.CTkSwitch(
            ir_row, text="Tăng cường", variable=ir_var,
            onvalue="on", offvalue="off",
    ...
    def toggle_feature(self, feature: str):
        ...
        if feature == 'face':
            ...
            log_activity(f"Nhận diện khuôn mặt {'bật' if face_on else 'tắt'} cho {self.source}", "info")
        ...
        if should_detect and not current_detect:
            ...
            log_activity(f"Tự động bật phát hiện cho {self.source}", "success")
        elif not should_detect and current_detect:
            self.state.set_detection(False, self.source)
            log_activity(f"Tự động tắt phát hiện cho {self.source} (Tất cả tính năng tắt)", "warning")
    ...
    def reconnect(self):
        if self.camera:
            ...
            log_activity(f"Đang kết nối lại Camera {self.source}", "info")
    ...
    def update_status(self):
        ...
        if connected:
            self.labels['status'].configure(text="Trực tuyến", text_color=Colors.SUCCESS)
            self.labels['dot'].configure(fg_color=Colors.SUCCESS)
            self.labels['progress'].set(1.0)
        else:
            self.labels['status'].configure(text="Ngoại tuyến", text_color=Colors.DANGER)
        ...
        if hasattr(self.camera, 'get_infrared_status'):
            ...
            # Cập nhật switch để đồng bộ với trạng thái thực
            self.switches['ir'].set("on" if is_ir else "off")
...
class CameraList(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        # Removed icon label
        ctk.CTkLabel(title_frame, text="Camera", font=Fonts.HEADING, text_color=Colors.TEXT_PRIMARY).pack(side="left")
        
        if self.on_add:
            create_button(header, "+ Thêm", "primary", "small", width=60, command=self.on_add).pack(side="right")
