    ...
    def update_video_feed(self):
        try:
            ...
            if ret:
                ...
                # Hiển thị thông báo khi camera mất kết nối
                if self.camera_loaded:
                    if getattr(self.camera, "is_reconnecting", False):
                        self.video_label.configure(image=None, text="Đang kết nối lại camera...")
                    else:
                        self.video_label.configure(image=None, text="Camera mất kết nối")
        except Exception as e:
            # Xử lý lỗi một cách im lặng để không làm gián đoạn GUI
            if self.video_label.winfo_exists():
                if getattr(self.camera, "is_reconnecting", False):
                    self.video_label.configure(image=None, text="Đang kết nối lại camera...")
                else:
                    self.video_label.configure(image=None, text="Lỗi camera")
