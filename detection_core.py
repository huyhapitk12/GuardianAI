...
from config import (
    ...
    CAMERA_AUTO_RECONNECT, CAMERA_RECONNECT_INTERVAL, CAMERA_MAX_FAILED_FRAMES
)
...
class Camera:
    def __init__(self, src=0, show_window=False):
        self.src = src  # Lưu nguồn camera để có thể kết nối lại
        ...
        self.auto_reconnect = CAMERA_AUTO_RECONNECT
        self.reconnect_interval = CAMERA_RECONNECT_INTERVAL
        self.max_failed_frames = CAMERA_MAX_FAILED_FRAMES
    ...
    def reconnect(self):
        """
        Thử kết nối lại camera. Trả về True nếu thành công, False nếu thất bại.
        """
        ...
        try:
            print(f"[Camera] Đang thử kết nối lại camera từ nguồn: {self.src}")
            ...
            if self.cap.isOpened():
                # Thử đọc một frame để xác nhận
                ret, _ = self.cap.read()
                if ret:
                    print(f"[Camera] Đã kết nối lại camera thành công!")
                    self.failed_frames_count = 0
                    self.is_reconnecting = False
                    return True
                else:
                    print(f"[Camera] Camera đã mở nhưng không đọc được frame.")
                    self.is_reconnecting = False
                    return False
            else:
                print(f"[Camera] Không thể mở camera/nguồn: {self.src}")
                self.is_reconnecting = False
                return False
                
        except Exception as e:
            print(f"[Camera] Lỗi khi kết nối lại camera: {e}")
            self.is_reconnecting = False
            return False
    ...
    def detect(self):
        ...
        while not self.quit:
            ...
            # Kiểm tra xem camera có còn mở không
            if self.cap is None or not self.cap.isOpened():
                if self.auto_reconnect:
                    self.failed_frames_count += 1
                    if self.failed_frames_count >= self.max_failed_frames:
                        print(f"[Camera] Camera đã mất kết nối. Đang thử kết nối lại...")
                        if self.reconnect():
                            continue
                        else:
                    ...
                    print("[Camera] Camera đã mất kết nối và tự động kết nối lại bị tắt.")
                    time.sleep(1.0)
                    continue
            ...
            try:
                ...
                print(f"[Camera] Lỗi khi grab frame: {e}")
                if self.auto_reconnect:
                    self.failed_frames_count += 1
                    if self.failed_frames_count >= self.max_failed_frames:
                        print(f"[Camera] Lỗi liên tục khi đọc frame. Đang thử kết nối lại...")
                        self.reconnect()
                    time.sleep(0.01)
                continue
            ...
            if not ret:
                if self.auto_reconnect:
                    self.failed_frames_count += 1
                    if self.failed_frames_count >= self.max_failed_frames:
                        print(f"[Camera] Không đọc được frame liên tục. Đang thử kết nối lại...")
                        if self.reconnect():
                            continue
                        else:
                            time.sleep(self.reconnect_interval)
            ...
            # Reset bộ đếm khi đọc frame thành công
            if ret:
                self.failed_frames_count = 0
