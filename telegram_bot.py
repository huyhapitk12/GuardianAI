...
def basic_confirm_stranger(user_response: str) -> str:
    """Xác nhận cơ bản dựa trên từ khóa 'có' hoặc 'không' trong câu trả lời."""
    text_lower = user_response.lower().strip()
    
    # Các từ khóa xác nhận "có"
    yes_keywords = ["có", "co", "đúng", "đúng rồi", "đúng vậy", "phải", "nhận ra", "biết", "quen", "người quen"]
    # Các từ khóa xác nhận "không"
    no_keywords = ["không", "khong", "ko", "không phải", "không biết", "không quen", "lạ", "người lạ"]
    
    # Kiểm tra từ khóa "có" trước
    for keyword in yes_keywords:
        if keyword in text_lower:
            return "yes"
    
    # Kiểm tra từ khóa "không"
    for keyword in no_keywords:
        if keyword in text_lower:
            return "no"
    
    # Nếu không tìm thấy từ khóa nào, trả về unknown
    return "unknown"
...
async def message_listener(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Xử lý xác nhận người lạ/người quen (cả khi AI_ENABLED True và False)
    if matched and matched['type'] in ('nguoi_quen', 'nguoi_la'):
        if AI_ENABLED:
            ...
            logger.info(f"Dùng xác nhận cơ bản cho cảnh báo {matched['id']}")
            decision = basic_confirm_stranger(text)
            logger.info(f"Xác nhận cơ bản đưa ra quyết định: '{decision}'")
        
        if decision in ("yes", "no"):
            if decision == "no":
                play_alarm()
                logger.warning(f"Xác nhận người lạ, KÍCH HOẠT CÒI BÁO ĐỘNG!")
            ...
            if AI_ENABLED:
                ...
                reply_text = f"✅ Đã ghi nhận: '{text}' (phân loại là '{decision}')."
            if decision == "no":
                reply_text += " Đã bật còi báo động!"
            await update.message.reply_text(reply_text)
        else: 
            await update.message.reply_text("🤔 Không hiểu câu trả lời của bạn. Vui lòng trả lời rõ hơn là 'có' hoặc 'không'.")
        return
